    ...
    <div>
        ...
        <form method="POST">
            <select name="id_medicament" required>
                <?php foreach ($meds_rows as $mr): ?>
                    <option value="<?= htmlspecialchars($mr['id'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($mr['nom'], ENT_QUOTES, 'UTF-8') ?></option>
                <?php endforeach; ?>
            </select>
        ...
        <table>
            ...
            if ($stocks_res) {
                while ($row = $stocks_res->fetch_assoc()) {
                    echo "<tr><td>" . htmlspecialchars($row['nom'], ENT_QUOTES, 'UTF-8') . "</td><td>" . htmlspecialchars($row['quantite'], ENT_QUOTES, 'UTF-8') . "</td></tr>";
                }
                $stocks_res->close();
            } else {
